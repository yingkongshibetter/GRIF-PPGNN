...
class Trainer:
    ...
    def fit(self,
        ...
        if fairchoice == True:
            ...
            for epoch in range(1, 10):
                ...
                train_metrics = self.loop(train_dataloader, stage='train-fair', prefix=prefix)
                ...
                if val_dataloader and self.val_interval and epoch % self.val_interval == 0:
                    ...
                    val_metrics = self.loop(val_dataloader, stage='val-fair', prefix=prefix)
                    metrics.update(val_metrics)

                    # print("metrics[val-fair/loss]",metrics["val-fair/loss"])
                    monitor_key = 'val-fair/loss'
                    if best_metrics is None or (metrics[monitor_key]< best_metrics[monitor_key]):
    ...
    def step(self, batch, stage: Stage, prefix: str) -> Metrics:
        ...
        if stage == 'train-fair':
            torch.set_grad_enabled(stage == 'train-fair')
        loss, metrics = self.model.step(batch, stage=stage)
        torch.set_grad_enabled(grad_state)

        if stage == 'train' or 'train-fair' and loss is not None:
            loss.backward()
            self.optimizer.step()
