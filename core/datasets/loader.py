...
from core.datasets.German import German
from core.datasets.Credit import Credit
from core.datasets.Income import Income
from core.datasets.Pokec import Pokec
...
class DatasetLoader:
    supported_datasets = {
        'Income': partial(Income,
            transform=Compose([
                ...
                FilterClassByCount(min_count=1, remove_unlabeled=True)
            ])
        ),
        'Pokec':partial(Pokec,
            transform=Compose([
                RandomNodeSplit(num_val=0.1, num_test=0.1),
                FilterClassByCount(min_count=1, remove_unlabeled=True)
            ])
        ),
    ...
    def load(self, verbose=False) -> Data:
        ...
        with open('/root/autodl-tmp/project/data.pkl', 'wb') as f:
            pickle.dump(data, f)
        return data
